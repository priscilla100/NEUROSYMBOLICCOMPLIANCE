...
% ============================================================================
\newtcolorbox{hipaabox}[1][]{
  colback=blue!5!white,
  colframe=blue!50!black,
  fonttitle=\bfseries,
  title={HIPAA Regulation Text},
  breakable,
  #1
}

\newtcolorbox{pdfbox}[1][]{
  colback=orange!5!white,
  colframe=orange!50!black,
  fonttitle=\bfseries,
  title={PDF Formal Notation},
  breakable,
  #1
}
...
% ============================================================================
\title{%
  \textbf{Formalizing the HIPAA Privacy Rule}\\
  \textbf{in Souffl\'{e} Datalog}\\[0.5em]
  \large A Comprehensive Guide to the ComplianceGPT Datalog Engine\\
  \large Covering \S\S 164.502, 164.506, 164.508, 164.510, 164.512, 164.514, and 164.524
}
\author{%
...
\begin{abstract}
This document provides a detailed, end-to-end explanation of how the HIPAA Privacy Rule was formalized in Souffl\'{e} Datalog. It covers \S 164.502 (General Rules), \S 164.506 (Treatment, Payment, and Health Care Operations), \S 164.508 (Authorization Required), \S 164.510 (Opportunity to Agree or Object), \S 164.512 (Uses and Disclosures Without Authorization), \S 164.514 (Other Requirements), and \S 164.524 (Individual Access). Starting from the source PDF formalization in first-order temporal logic, we describe every design decision, type declaration, hierarchy, helper predicate, and rule translation. The document covers the architecture of the compliance checker, the ADT-based explanation tree mechanism for provenance, the handling of stratified negation, oracle predicates, grounding issues, testing methodology, and a complete walkthrough of test scenarios. The intended audience is a student learning to understand and extend this formalization.
\end{abstract}
...
\subsection{What are \S 164.502 and \S 164.506?}

\begin{description}
  \item[\S 164.502 --- General Rules:] The gatekeeper section. It specifies the general principle that a covered entity may not use or disclose PHI \emph{except} as permitted or required by the Privacy Rule. It enumerates all permitted and required disclosure pathways (subsections (a)(1)(i) through (a)(1)(vi) for permitted uses, (a)(2) for required disclosures), plus constraints and special cases (minimum necessary in (b), restriction agreements in (c), de-identified info in (d), business associates in (e), personal representatives in (g), whistleblower/crime victim in (j)).

  \item[\S 164.506 --- Treatment, Payment, or Health Care Operations (TPO):] The implementation specification for TPO disclosures. When \S 164.502(a)(1)(ii) permits a disclosure ``for treatment, payment, or health care operations, as permitted by and in compliance with \S 164.506,'' control passes to this section. It has three main parts: (a) standard permitted uses (no authorization required), (b) consent for TPO, and (c) the implementation specification with five sub-rules (c)(1)--(c)(5) covering different TPO scenarios.
\end{description}
...
\end{lstlisting}
  ...
  The constraint is used \emph{inside} other rules (e.g., \S 502(a)(1)(iii) requires \texttt{minimum\_necessary\_satisfied(ARGS)} in its body) but does not appear as a clause of \texttt{permitted\_by\_164\_502}.
\end{description}
...
\end{forest}
\caption{Role hierarchy for covered entities. Arrows point from child to parent (specialization).}
...
\midrule
0 & Base facts: \texttt{activerole}, \texttt{msg\_contains}, \texttt{disclosure\_attempted}, \\
  & oracle predicates, hierarchy base facts \\
1 & Positive rules: hierarchy closures (\texttt{role\_subtype}, \texttt{attr\_subtype}, \\
  & \texttt{purpose\_subtype}), helper predicates (\texttt{has\_role}, \texttt{is\_covered\_entity}, \\
  & \texttt{is\_phi}), and the stub declarations \\
2 & Negation rules: \texttt{permitted\_by\_164\_506\_a} (negates \texttt{require\_authorization\_by\_164\_508}), \\
  & \texttt{permitted\_by\_164\_502\_e} (negates \texttt{excluded\_164\_502\_e\_1\_ii}), \\
  & \texttt{is\_personal\_representative} (negates \texttt{abuse\_exception}), \\
  & \texttt{is\_disclosure\_denied} (negates \texttt{is\_disclosure\_allowed}) \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item \textbf{Personal representative}: \texttt{!abuse\_exception(P, Q)} and \texttt{!minor\_acts\_as\_individual(P, Q)} --- the representative status is blocked if abuse or minor exception applies.
...
\toprule
\textbf{Prolog (PDF)} & \textbf{Souffl\'{e}} & \textbf{Notes} \\
\midrule
\texttt{fail} & Omit the rule & A rule that always fails is simply not written. \\
\texttt{writeln('...')} & \texttt{\$Leaf("...")} or \texttt{\$Step1("...", E)} & Debug output becomes ADT explanation reason string. \\
\texttt{\textbackslash+} (negation) & \texttt{!} & Prolog negation-as-failure becomes Souffl\'{e} stratified negation. \\
\texttt{;} (disjunction) & \texttt{;} or multiple rules & Souffl\'{e} supports \texttt{;} in rule bodies, or use separate rules for the same head. \\
\texttt{debug(...)} & Comment & PDF debug calls become comments in the Souffl\'{e} code. \\
Bundled \texttt{A} & \texttt{(p1, p2, q, m, t, u)} & The single parameter is unpacked into 6 variables. \\
\texttt{is\_from\_coveredEntity(A)} & \texttt{is\_covered\_entity(p1)} & Macro expanded to check \texttt{p1}'s role. \\
\texttt{is\_phi(A)} & \texttt{is\_phi(t)} & Macro expanded to check attribute type. \\
\texttt{is\_for\_eitherPurpose(A)} & \texttt{is\_for\_tpo(u)} & Macro renamed and expanded. \\
\texttt{is\_to\_healthCareProvider(A)} & \texttt{is\_health\_care\_provider(p2)} & Macro expanded to check \texttt{p2}'s role. \\
\texttt{is\_to\_concernedIndividual(A)} & \texttt{(p2 = q ; is\_personal\_representative(p2, q))} & ``Concerned individual'' unpacked. \\
\texttt{activerole(p, role)} & \texttt{activerole(P, "role")} or \texttt{has\_role(P, "role")} & Direct or hierarchy-aware lookup. \\
\texttt{contains(m, (q, t))} & \texttt{msg\_contains(m, q, t)} & Renamed (``contains'' is reserved in Souffl\'{e}). \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}
  ...
  \item \textbf{Testing}: 177 test cases (124 systematic, 25 realistic, 28 GoldCoin court cases) validate the formalization against expected outcomes and real legal decisions.
\end{enumerate}
